...
class myUnet(object):
	...
	def train(self):
		...
		"""
		model_checkpoint = ModelCheckpoint('unet_bladder_test.hdf5', monitor='val_acc', verbose=1, save_best_only=True)
		print(('Fitting model...'))
		#set epochs back to 50
		model.fit(imgs_train, imgs_mask_train, batch_size=4, epochs=3, verbose=1,
				  validation_split=0.1, shuffle=True, callbacks=[model_checkpoint])
		
		"""
		
		#problem: Keras sets the validation set to be the last 10% of train
		#		  this can cause issues, as possibly only one slide will be represented here
		#Soltution: Shuffle images/masks first
		
		"""
		imgs_train_h1 = imgs_train[0:len(imgs_train)/2]
		imgs_train_h2 = imgs_train[len(imgs_train)/2:len(imgs_train)]
		imgs_mask_train_h1 = imgs_mask_train[0:len(imgs_train)/2]
		imgs_mask_train_h2 = imgs_mask_train[len(imgs_train)/2:len(imgs_train)]
		imgs_train = imgs_train_h2 + imgs_train_h1
		imgs_mask_train = imgs_mask_train_h2 + imgs_mask_train_h1
		"""
		
		callback = [EarlyStopping(monitor='val_acc', patience=3),
            ModelCheckpoint('unet_bladder_test.hdf5', monitor='val_acc', verbose=1, save_best_only=True)]
		...
		np.save('./results_test_2/bladder_mask_test.npy', imgs_mask_test)

	def save_img(self):
		print(("array to image"))
		imgs = np.load('./results_test_2/bladder_mask_test.npy')
		piclist = []
		for line in open("./results_test_2/bladder_test.txt"):
			line = line.strip()
			picname = line.split('/')[-1]
			piclist.append(picname)
		for i in range(imgs.shape[0]):
			path = "./results_test_2/" + piclist[i]
	...
	def save_val_img(self):
		print(("array to image"))
		imgs = np.load('./results_val/test/bladder_mask_test.npy')
		piclist = []
		for line in open("./results_val/test/bladder_test.txt"):
			line = line.strip()
			picname = line.split('/')[-1]
			piclist.append(picname)
		for i in range(imgs.shape[0]):
			path = "./results_val/10/he_n/" + piclist[i]
			img = np.zeros((imgs.shape[1], imgs.shape[2], 3), dtype=np.uint8)
			for k in range(len(img)):
				for j in range(len(img[k])):
					num = np.argmax(imgs[i][k][j])
