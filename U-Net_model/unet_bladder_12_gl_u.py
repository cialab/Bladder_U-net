...
class myUnet(object):
	...
	def get_unet(self):
		inputs = Input((self.img_rows, self.img_cols, 3)) #I guess 3 stands for R.B.G ? -Y.Z
	...
	def train(self):
		...
		print(("got unet"))
		"""
		model_checkpoint = ModelCheckpoint('unet_bladder_12_gl_u.hdf5', monitor='val_acc', verbose=1, save_best_only=True)
		print(('Fitting model...'))
		...
		callback = [ModelCheckpoint('unet_bladder_12_gl_u.hdf5', monitor='val_acc', verbose=1, save_best_only=True)]
		...
		np.save('./results/12/gl_u/bladder_mask_test.npy', imgs_mask_test)

	def save_img(self):
		print(("array to image"))
		imgs = np.load('./results/12/gl_u/bladder_mask_test.npy')
		piclist = []
		for line in open("./results/bladder.txt"):
			line = line.strip()
			picname = line.split('/')[-1]
			piclist.append(picname)
		for i in range(imgs.shape[0]):#for each image -Y.Z
			path = "./results/12/gl_u/" + piclist[i]
			img = np.zeros((imgs.shape[1], imgs.shape[2], 3), dtype=np.uint8)
            #make labels pixle by pixle -Y.Z
	...
	def save_val_img(self):
		print(("array to image"))
		imgs = np.load('./results_val/12/gl_u/bladder_mask_test.npy')
		piclist = []
		...
		for i in range(imgs.shape[0]):
			path = "./results_val/12/gl_u/" + piclist[i]
			img = np.zeros((imgs.shape[1], imgs.shape[2], 3), dtype=np.uint8)
			for k in range(len(img)):
				for j in range(len(img[k])):
					num = np.argmax(imgs[i][k][j])
