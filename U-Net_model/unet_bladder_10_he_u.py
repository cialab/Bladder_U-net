...
class myUnet(object):
	...
	def train(self):
		...
		model = self.get_unet()
		print(("got unet"))
		#model_checkpoint = ModelCheckpoint('unet_bladder_10_he_u.hdf5', monitor='val_acc', verbose=1, save_best_only=True)
		
		#EarlyStopping(monitor='val_acc', patience=3),
		callback = [ModelCheckpoint('unet_bladder_10_he_u.hdf5', monitor='val_acc', verbose=1, save_best_only=True)]
		...
		np.save('./results/10/he_u/bladder_mask_test.npy', imgs_mask_test)

	def save_img(self):
		print(("array to image"))
		imgs = np.load('./results/10/he_u/bladder_mask_test.npy')
		piclist = []
		...
		for i in range(imgs.shape[0]):
			path = "./results/10/he_u/" + piclist[i]
	...
	def save_val_img(self):
		print(("array to image"))
		imgs = np.load('./results_val/10/he_u/bladder_mask_test.npy')
		piclist = []
		for line in open("./results_val/bladder.txt"):
			...
			piclist.append(picname)
		for i in range(imgs.shape[0]):
			path = "./results_val/10/he_u/" + piclist[i]
			img = np.zeros((imgs.shape[1], imgs.shape[2], 3), dtype=np.uint8)
			for k in range(len(img)):
				for j in range(len(img[k])):
					num = np.argmax(imgs[i][k][j])
