...
class myUnet(object):
	...
	def train(self):
		...
		callback = [ModelCheckpoint('unet_bladder_10_he_n.hdf5', monitor='val_acc', verbose=1, save_best_only=True)]
		...
		np.save('./results/10/he_n/bladder_mask_test.npy', imgs_mask_test)

	def save_img(self):
		print(("array to image"))
		imgs = np.load('./results/10/he_n/bladder_mask_test.npy')
		piclist = []
		...
		for i in range(imgs.shape[0]):
			path = "./results/10/he_n/" + piclist[i]
	...
	def save_val_img(self):
		print(("array to image"))
		imgs = np.load('./results_val/10/he_n/bladder_mask_test.npy')
		piclist = []
